...
void initialize(int threshold) {
	...
	//getting generators and pre-computing pairing of it
	ep_null(g1);
	ep2_null(g2);
	ep2_null(gAlpha);
	ep_new(g1);
...
static void throw_illegal_state_exception(JNIEnv *env, char *message) {
...
static bn_t *bn_custom_invert(bn_t number) {
	bn_t *result = malloc(sizeof(bn_t));
	bn_null(*result);
	bn_new(*result);
...
JNIEXPORT jboolean JNICALL Java_vss_commitment_constant_Pairing_verifyWithoutPreComputation (JNIEnv *env, jobject obj,
    ...
    fp12_t sharePairing, witnessPairing, rightSide, commitmentP;
    fp12_null(sharePairing);
    fp12_null(witnessPairing);
    fp12_null(rightSide);
    fp12_null(commitmentP);
    fp12_new(sharePairing);
    fp12_new(witnessPairing);
    fp12_new(rightSide);
    fp12_new(commitmentP);
...
//    ep2_sub_basic(gAlphaI, gAlpha, gI);
    ...
    fp12_mul_basic(rightSide, witnessPairing, sharePairing);

    int cmp = fp12_cmp(commitmentP, rightSide);
...
static bn_t *multiply_polynomials(bn_t *values_1, bn_t *values_2, int n_values_1, int n_values_2) {
